  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-1 no-scrollbar md:px-6 md:max-w-xl md:mx-auto md:w-full">
        ...
        {[
          'Bạn là chuyên gia tổng hợp thông t..',
          'Tra cứu mã số Bảo hiểm xã hội',
          'Tôi muốn biết có bao nhiêu con Mèo',
          'Bạn là chuyên gia tư vấn bảo hiểm hôn nhân',
        ].map((title, i) => (
          <button
            key={i}
            onClick={() => navigate({ to: '/' })}
            className="flex items-center gap-2 px-3 py-2.5 text-left rounded-lg hover:bg-muted transition-colors"
          >
            <span className="text-[15px] flex-1 truncate">{title}</span>
            {i === 3 && <Pin size={13} className="text-info shrink-0" />}
          </button>
        ))}
      </div>
    </>
